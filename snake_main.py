...
#蛇类
#点以25为单位
#继承object类是新式类，不继承object类是经典类，在python3.x中无论父类是否继承object
#子类中调用重写过的父类中的方法时遵循广度优先的方法，调用子类最临近的父类中的方法
class Snake(object):
    #初始化各种需要的属性 
    #一个类的构造函数，当创建这个类的实例就会调用该方法 __init__(self) self代表类的实例
    def __init__(self):
    ...
    #蛇前进时每次在前方25处增加一个小方块，同时删除尾部的一个块
    def addnode(self):
        left,top=(0,0)
        if self.body:
